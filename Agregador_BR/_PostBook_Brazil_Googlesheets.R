...
rm(list=ls(all=TRUE))
if(length(grep("(Z|z)ucco",setwd("~")))==1){
  the.path <- "~/Dropbox/Data/Popularity-PostBook"
}else{
  the.path <- "~/Dropbox/Data/Popularity-PostBook"
}
setwd(the.path)
...
d$raw.date <- NULL
save(d,file="DATA/popularity_raw_BR.RData")

#fork: save Bolsonaro's popularity separately 
dd <- subset(d,President=="Bolsonaro"&year>=2019)
save(dd,file="DATA/popularity_raw_bolsonaro_BR.RData") 
...
load("DATA/popularity_raw_BR.RData")
